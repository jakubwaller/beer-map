...
# it has been rested — the cool-down doubles with the latter. A resting host
# is skipped without a single packet; when every configured host is resting,
# fetch_overpass raises OverpassUnavailable at once and the caller can wait
# breaker_wait_s() instead of failing tile after tile in milliseconds. See
# config.OVERPASS_TRIP_* for the numbers and the night that earned them.
_breaker: dict[str, dict] = {}


class OverpassUnavailable(httpx.RequestError):
    """Every configured host is resting after failures; nothing was contacted."""

    def __init__(self, message: str):
        super().__init__(message, request=httpx.Request("GET", "https://overpass"))
    ...
    for url in urls:
        ...
        for attempt in range(retries):
            ...
            try:
                ...
                # The port never opened — refused, unreachable, no route: what
                # a firewall ban (or a dead host) looks like. It will not open
                # in five seconds either, and knocking three more times is
                # exactly what a ban is meant to discourage. Rest the host.
                last_exc = exc
