...
A tap is resolved by proximity, not by landing on the glyph: every drawn dot registers as a tap target carrying its radius, and the nearest one within the slop of its edge opens (`TOUCH_SLOP` 22px, `MOUSE_SLOP` 6px, picked per click from the click's own `pointerType` — a touchscreen laptop has both pointers and calls itself the mouse, so the device is the wrong thing to ask). Hitting a 16px venue dot, or a 4px gray one, is otherwise a game of skill on a phone. Three things this must keep doing:
...
- Let a tap that **dismisses** the suggestion list do only that. With the slop this wide there is barely a spot in a city that is not near some dot, so a dismissing tap would otherwise always come with a venue attached.
