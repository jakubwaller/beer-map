...
```bash
git fetch origin
git worktree add -b <branch> ~/gitlab/.worktrees/beermap-<task> origin/main
```
...
`osm.py` also has a circuit breaker, and it is deliberately blunt: a host whose port refuses the connection is rested on the spot with a doubling cool-down (15 min → 2 h), three queries in a row exhausting their retries rest it too, and when every host is resting `fetch_overpass` raises `OverpassUnavailable` without a packet — `country.py` then waits for the first host to come back (three times, then stops with a `--resume` hint) instead of quartering tiles that were never asked. Earned on 2026-08-23, when Overpass had banned the host's IPv4 and the sweep kept knocking 1,500 times. Don't trade it for more retries.
...
No build step, no npm deps: vanilla ES modules + vendored MapLibre (`web/vendor/`). The pure functions live in `datasource.js` (load/dedupe/filter, the folded token-scored venue search, and the brand chip/picker lists), `hours.js` (the OSM `opening_hours` subset parser, "open now" and the German week view) and `hittest.js` (nearest-tap-target maths) — those are the unit-tested part, via `node --test web/*.test.js`. `app.js` does the map, filter UI, search dropdown, brand autocomplete, and submission forms.

A tap is resolved by proximity, not by landing on the glyph: every drawn dot registers as a tap target carrying its radius, and the nearest one within the slop of its edge opens (`TOUCH_SLOP` 22px, `MOUSE_SLOP` 6px, picked per click from the `pointerType` of the `pointerdown` that caused it — a touchscreen laptop has both pointers and calls itself the mouse, so the device is the wrong thing to ask, and the click's own label is not trustworthy on every engine). Hitting a 16px venue dot, or a 4px gray one, is otherwise a game of skill on a phone. The dismissal and hold rules below are about fingers only: within its 6px slop a mouse click counts as a direct hit, so it neither waits nor defers to a dismissal. Desktop is not left untouched by that — a click 6px off a gray dot now opens it, and a click that dismisses the dropdown can open a venue with it — but at that distance the pointer was on the dot. What this must keep doing:
...
- Let a finger that **dismisses** the suggestion list do only that — unless it came down inside a dot's glyph, which is unambiguous and opens it, the same answer the dot's own marker gives. That exception is real for a 16px venue dot and a cluster; a gray dot is 4px across at zoom 10, so at low zoom a finger cannot land inside one and the dismissal wins. With the slop this wide there is barely a spot in a city that is not near some dot, so a dismissing tap would otherwise always come with a venue attached.
- Open **nothing** for a finger that landed while the map was still gliding under the user's own fling (`userMoving && map.isMoving()` at `pointerdown`, marker `onclick` included): that tap was there to stop the glide, and it would otherwise open whichever pub slid under the finger. A programmatic easeTo or flyTo does not count — a tap during one of those is aiming at a dot.
- **Hold** a finger's near miss for `NEAR_TAP_HOLD_MS` and drop it if any new press follows (a document-level `pointerdown`, so a hand moving on to the search box or a chip counts): a double tap is how you zoom in, and its first tap arrives here as an ordinary click. A tap that landed *on* a dot is never held — it is unambiguous, and holding it would make the common case feel slow.
