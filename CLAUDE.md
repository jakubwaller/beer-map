...
- Let a tap that **dismisses** the suggestion list do only that — unless it came down squarely on a dot, which is unambiguous and opens it, the same answer the dot's own marker gives. With the slop this wide there is barely a spot in a city that is not near some dot, so a dismissing tap would otherwise always come with a venue attached.
