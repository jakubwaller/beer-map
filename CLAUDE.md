...
A tap is resolved by proximity, not by landing on the glyph: every drawn dot registers as a tap target carrying its radius, and the nearest one within the slop of its edge opens (`TOUCH_SLOP` 22px, `MOUSE_SLOP` 6px, picked per click from whether a finger aimed it — a touchscreen laptop has both pointers and calls itself the mouse). Hitting a 16px venue dot, or a 4px gray one, is otherwise a game of skill on a phone. Three things this must keep doing:
