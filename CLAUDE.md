...
A tap is resolved by proximity, not by landing on the glyph: every drawn dot registers as a tap target carrying its radius, and the nearest one within `TAP_SLOP` of its edge (22px for a fingertip, 6 for a mouse) opens — hitting a 16px venue dot, or a 4px gray one, is otherwise a game of skill on a phone. Two things this must keep doing: measure to the dot's *edge*, so a 56px cluster does not lose the tap to a small dot nearer its centre; and aim at where the finger came **down** (the `touchstart` lngLat, re-projected at click time), not where the click is reported. A tap that smears a few pixels drags the map along under it, and aiming at the release pixel then opens the neighbouring pub.
