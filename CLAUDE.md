...
A tap is resolved by proximity, not by landing on the glyph: every drawn dot registers as a tap target carrying its radius, and the nearest one within the slop of its edge opens (`TOUCH_SLOP` 22px, `MOUSE_SLOP` 6px, picked per click from the `pointerType` of the `pointerdown` that caused it — a touchscreen laptop has both pointers and calls itself the mouse, so the device is the wrong thing to ask, and the click's own label is not trustworthy on every engine). Hitting a 16px venue dot, or a 4px gray one, is otherwise a game of skill on a phone. What this must keep doing:
...
- Let a tap that **dismisses** the suggestion list do only that — unless it came down inside a dot's glyph, which is unambiguous and opens it, the same answer the dot's own marker gives. That exception is real for a 16px venue dot and a cluster; a gray dot is 2px at zoom 10, so at low zoom a finger cannot land inside one and the dismissal wins. With the slop this wide there is barely a spot in a city that is not near some dot, so a dismissing tap would otherwise always come with a venue attached.
