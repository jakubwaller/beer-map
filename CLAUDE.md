...
A tap is resolved by proximity, not by landing on the glyph: every drawn dot registers as a tap target carrying its radius, and the nearest one within the slop of its edge opens (`TOUCH_SLOP` 22px, `MOUSE_SLOP` 6px, picked per click from the click's own `pointerType` — a touchscreen laptop has both pointers and calls itself the mouse, so the device is the wrong thing to ask). Hitting a 16px venue dot, or a 4px gray one, is otherwise a game of skill on a phone. What this must keep doing:
...
- Open **nothing** for a press that landed while the map was still gliding (`map.isMoving()` at `pointerdown`): that tap was there to stop the glide, and it would otherwise open whichever pub slid under the finger.
